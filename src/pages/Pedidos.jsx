...
const STATUS_COR = {
  ...
  concluido: 'bg-onforge-black',
  exportado: 'bg-onforge-gray',
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
        <h1 className="text-3xl font-bold font-display">{isAdmin ? 'Pedidos' : 'Meus Pedidos'}</h1>
        <div className="flex gap-2">
          <button onClick={() => navigate('/pedidos/novo')} className="bg-onforge-black text-white px-4 py-2 rounded hover:bg-black/80">
            + Novo Pedido
          </button>
          {isAdmin && (
      ...
      <form onSubmit={aplicarFiltros} className="bg-white rounded-lg shadow p-4 mb-6 flex flex-wrap gap-3 items-end">
        ...
        <div>
          <label className="block text-xs font-medium text-onforge-black/80 mb-1">Status</label>
          <select
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-onforge-black/50">Nenhum pedido encontrado</div>
      ) : (
                ...
                return (
                  <tr key={p.id} className="border-b hover:bg-onforge-cream/60">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-white text-xs ${STATUS_COR[p.status] || 'bg-onforge-gray'}`}>
                    ...
                    <td className="px-4 py-3 space-x-3 whitespace-nowrap">
                      <button onClick={() => abrirDetalhe(p)} className="text-onforge-black hover:opacity-70 text-sm">Ver</button>
                      {edicaoBloqueada ? (
                        <span className="text-onforge-gray text-sm cursor-not-allowed">Editar</span>
                      ) : (
                        <button onClick={() => navigate(`/pedidos/${p.id}/editar`)} className="text-onforge-black hover:opacity-70 text-sm">
                          Editar
                        </button>
                      )}
      ...
      <Modal isOpen={!!detalhe} title={detalhe ? `Pedido nº ${detalhe.numero}` : ''} onClose={() => setDetalhe(null)}>
        {detalhe && (
          <div>
            <p className="text-sm text-onforge-black/60 mb-1">Cliente: <strong>{detalhe.cliente_nome}</strong></p>
            <p className="text-sm text-onforge-black/60 mb-1">Vendedor: <strong>{detalhe.vendedor_nome}</strong></p>
            <p className="text-sm text-onforge-black/60 mb-1">Data: <strong>{formatDate(detalhe.data_pedido)}</strong></p>
            <p className="text-sm text-onforge-black/60 mb-1">
              Status: <span className={`px-2 py-1 rounded text-white text-xs ${STATUS_COR[detalhe.status] || 'bg-onforge-gray'}`}>
                {STATUS_LABEL[detalhe.status] || detalhe.status}
              </span>
            </p>
            {detalhe.desconto_geral > 0 && (
              <p className="text-sm text-onforge-black/60 mb-1">Desconto Geral: <strong>{detalhe.desconto_geral}%</strong></p>
            )}
            {detalhe.condicao_pagamento && (
              <p className="text-sm text-onforge-black/60 mb-1">Condição Pagamento: <strong>{detalhe.condicao_pagamento}</strong></p>
            )}
            {detalhe.observacao_pagamento && (
              <p className="text-sm text-onforge-black/60 mb-3">Observação: <strong>{detalhe.observacao_pagamento}</strong></p>
            )}
